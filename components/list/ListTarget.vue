...
const {data: targets} = await useAsyncData('targets', () => projectApi.getTargets(projectStore.project.id))
...
<template>
    ...
    />
    <div class="min-h-[300px]"> <!-- Add this wrapper div with minimum height -->
      <UTable ref="table" v-model:column-filters="columnFilters" :data="tableData" :columns="columns" />
    </div>
  </template>